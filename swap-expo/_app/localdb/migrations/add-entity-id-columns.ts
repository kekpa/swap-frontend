...
export async function runMigration(db: SQLiteDatabase): Promise<void> {
  ...
  try {
    ...
    logger.info('[Migration] Step 1: Adding entity_id columns...');
    ...
    try {
      ...
      logger.warn('[Migration] Messages table may not exist, skipping');
    }
    ...
    try {
      ...
      logger.warn('[Migration] interaction_members table may not exist, skipping');
    }
    ...
    try {
      ...
      logger.warn('[Migration] Timeline table may not exist, skipping');
    }
    ...
    try {
      ...
      logger.warn('[Migration] Notifications table may not exist, skipping');
    }
    ...
    try {
      ...
      logger.warn('[Migration] user_contacts table may not exist, skipping');
    }
    ...
    logger.info('[Migration] Step 2: Copying data from profile_id to entity_id...');
    ...
    try {
      ...
      logger.debug('[Migration] Copied profile_id to entity_id in users table');
    } catch (error) {
      logger.warn('[Migration] Could not copy users.profile_id to entity_id:', error);
    }
    ...
    try {
      ...
      logger.debug('[Migration] Copied profile_id to entity_id in kyc_status table');
    } catch (error) {
      logger.warn('[Migration] Could not copy kyc_status.profile_id to entity_id:', error);
    }
    ...
    try {
      ...
      logger.debug('[Migration] Copied profile_id to entity_id in transactions table');
    } catch (error) {
      logger.warn('[Migration] Could not copy transactions.profile_id to entity_id:', error);
    }
    ...
    try {
      ...
      logger.debug('[Migration] Copied profile_id to entity_id in interactions table');
    } catch (error) {
      logger.warn('[Migration] Could not copy interactions.profile_id to entity_id:', error);
    }
    ...
    try {
      ...
      logger.debug('[Migration] Copied profile_id to entity_id in interaction_members table');
    } catch (error) {
      logger.warn('[Migration] Could not copy interaction_members.profile_id to entity_id:', error);
    }
    ...
    try {
      ...
      logger.debug('[Migration] Copied profile_id to entity_id in currency_wallets table');
    } catch (error) {
      logger.warn('[Migration] Could not copy currency_wallets.profile_id to entity_id:', error);
    }
    ...
    logger.info('[Migration] Step 3: Creating indexes...');
    ...
    await db.execAsync(`
      ...
      ON users(entity_id);
    `);
    logger.debug('[Migration] Created index on users.entity_id');
    ...
    `);
    logger.debug('[Migration] Created index on kyc_status.entity_id');
    ...
    `);
    logger.debug('[Migration] Created index on transactions.entity_id');
    ...
    try {
      ...
      `);
      logger.debug('[Migration] Created index on messages.entity_id');
    } catch (error) {
      logger.warn('[Migration] Could not create index on messages.entity_id');
    }
    ...
    `);
    logger.debug('[Migration] Created index on interactions.entity_id');
    ...
    `);
    logger.debug('[Migration] Created index on currency_wallets.entity_id');
    ...
    try {
      ...
      `);
      logger.debug('[Migration] Created index on timeline.entity_id');
    } catch (error) {
      logger.warn('[Migration] Could not create index on timeline.entity_id');
    }
    ...
    logger.info('[Migration] Step 4: Recording migration...');
    ...
    try {
      ...
      logger.error('[Migration] Migration rolled back due to error');
    } catch (rollbackError) {
