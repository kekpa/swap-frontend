...
import { saveAccessToken, saveRefreshToken, getAccessToken, getRefreshToken, clearTokens } from '../utils/tokenStorage';
...
class TokenManager {
    ...
    try {
      ...
      if (accessToken) {
        ...
        if (metadata && !this.isTokenExpired(metadata)) {
          ...
          logger.warn('[TokenManager] ⚠️ Access token in storage is expired or invalid');
        }
      }
  ...
  getCurrentAccessToken(): string | null {
    ...
    if (this.shouldRefreshToken(this.accessTokenCache)) {
      logger.debug('[TokenManager] ⚠️ Token expires soon, should refresh', 'token_manager', {
